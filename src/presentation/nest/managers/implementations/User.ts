...
import { AbstractCreateUserUseCase, CreateUserResponse } from "./../../../../application/useCases/user/Create/AbstractCreateUser"
...
export class UserManager implements AbstractUserManager {
  ...
  async create(createUserRequestDto: CreateUserRequestDto): Promise<CreateUserResponse> {
    const createUserUseCase: AbstractCreateUserUseCase = new CreateUserUseCase(this.userRepository, this.passwordHasher)
   ...
   */
  async getAll(readUsersRequestDto: ReadUsersRequestDto): Promise<GetUsersResponse> {
